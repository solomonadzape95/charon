...
export async function GET() {
  ...
  const [usersC, creators, seriesC, chaptersC, payments, deposits, recentPays, recentSess] = await Promise.all([
    ...
    db.from("payments").select("amount_usdc, status, created_at").limit(10000),
    db.from("ledger").select("amount_usd, kind").eq("kind", "deposit").limit(10000),
    db.from("payments").select("id, amount_usdc, status, created_at, creator_id, chapter_id").order("created_at", { ascending: false }).limit(8),
  ...
  const payRows = (payments.data ?? []) as { amount_usdc: number; status: string; created_at: string }[];
  ...
  return NextResponse.json({
    ...
    money: {
      totalSettled,
      settledToday: sumSince(DAY),
      settledWeek: sumSince(7 * DAY),
      platformFee: Math.round(totalSettled * 0.05 * 100) / 100,
      escrowHeld: escrow,
      lifetimeEarned,
      totalDeposits,
